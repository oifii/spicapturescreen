...
// spicapturescreen.cpp : Defines the entry point for the console application.
//

#include "stdafx.h"
#include "windows.h" // should be less than and greater than instead of \"

WCHAR global_filename[1024];
...
{
	wcscpy(global_filename, L"output.bmp");
	if(argc>1)
	{
		wcscpy(global_filename, argv[1]);

	}
	CaptureBMP(global_filename);
	return 0;
}
